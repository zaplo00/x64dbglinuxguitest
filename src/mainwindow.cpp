...
{
    /*setGlobalShortcut(ui->actionOpen, NULL);
...
{
    AboutDialog dialog(this); //mUpdateCheker
    dialog.exec();
}

void MainWindow::openFileSlot()
{
    auto filename = QFileDialog::getOpenFileName(this, tr("Open file"), NULL/*mMRUList->getEntry(0)*/, tr("Executables (*.exe *.dll);;All files (*.*)"));
    if(!filename.length())
        return;
